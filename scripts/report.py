...
def load_action_log():
    """Load open actions from Action Log sheet."""
    try:
        creds = Credentials(
            token=None,
            refresh_token=GMAIL_REFRESH_TOKEN,
        ...
        sh = gc.open_by_key(SHEETS_ID)
        ws = sh.worksheet("Action Log")
        records = ws.get_all_records()
        # Return dict keyed by (account_name, issue_type) -> record
        open_issues = {}
        for r in records:
            if str(r.get("Status","")).lower() == "open":
                key = (r.get("Account Name",""), r.get("Issue Type",""))
                open_issues[key] = r
        return open_issues, ws
    except Exception as e:
        print(f"   Could not load Action Log: {e}")
        return {}, None


def log_action(ws, account_name, issue_type, metric, card_id, card_url=""):
    """Log a new card to the Action Log sheet."""
    try:
        ws.append_row([
        ...
        print(f"   Could not log action: {e}")


def add_trello_comment(card_id, comment):
...
def sync_trello_closed_cards(open_issues, ws):
    """Check if any open Trello cards have been moved to Done/Won't Do and update sheet."""
    done_lists = [
        "6a27b00d4c7713018054ab7d",  # Won't Do
        "6a27b00d68c1d04fe1263a9e",  # Done
    ]
    try:
        records = ws.get_all_records()
        for i, r in enumerate(records):
            if str(r.get("Status","")).lower() != "open":
                continue
            card_id = r.get("Trello Card ID","")
            if not card_id:
                continue
            # Check card's current list
            resp = requests.get(
                f"https://api.trello.com/1/cards/{card_id}",
                params={"key": TRELLO_KEY, "token": TRELLO_TOKEN_VAL, "fields": "idList"}
            )
            if resp.status_code == 200:
                current_list = resp.json().get("idList","")
                if current_list in done_lists:
                    row_num = i + 2
                    ws.update([[
                        "closed"
                    ]], f"G{row_num}")
                    ...
                    print(f"   Closed action: {r.get('Account Name')} - {r.get('Issue Type')}")
    except Exception as e:
        print(f"   Could not sync closed cards: {e}")
...
def create_tasks_in_trello(all_results, team):
    ...
    # Load action log to avoid duplicate cards
    open_issues, action_log_ws = load_action_log()
    if action_log_ws:
        sync_trello_closed_cards(open_issues, action_log_ws)
        # Reload after sync
        open_issues, action_log_ws = load_action_log()
...
*Auto-generated by Carousel Media Reporting System*"""
            ...
            issue_key = (account_name, f"alert_{alert['type']}")
            if issue_key in open_issues:
                # Card already exists — add a comment with today's update
                existing = open_issues[issue_key]
                existing_card_id = existing.get("Trello Card ID","")
                if existing_card_id:
                    comment = f"**Update {date_str}:** {message}\n\nROAS: {y.get('roas',0)}x | Spend: ₹{y.get('spend',0):,.0f} | Revenue: ₹{y.get('revenue',0):,.0f}"
                    add_trello_comment(existing_card_id, comment)
                    print(f"   Updated existing card for: {account_name} - {alert['type']}")
                continue
            ...
            if card and card.get("id"):
                ...
                if action_log_ws:
                    log_action(action_log_ws, account_name, f"alert_{alert['type']}",
                              alert.get("metric",""), card["id"],
                              card.get("shortUrl",""))
...
*Auto-generated by Carousel Media Reporting System*"""
            ...
            issue_key = (account_name, itype)
            if issue_key in open_issues:
                existing = open_issues[issue_key]
                existing_card_id = existing.get("Trello Card ID","")
                if existing_card_id:
                    comment = f"**Update {date_str}:** {insight.get('text','')[:200]}\n\nROAS 7D: {l7.get('roas',0)}x | Spend 7D: ₹{l7.get('spend',0):,.0f}"
                    add_trello_comment(existing_card_id, comment)
                    print(f"   Updated existing card for: {account_name} - {itype}")
                continue
            ...
            if card and card.get("id"):
                ...
                if action_log_ws:
                    log_action(action_log_ws, account_name, itype,
                              insight.get("title",""), card["id"],
                              card.get("shortUrl",""))
